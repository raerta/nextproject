...
const CreatePost = () => {
  ...
  const { postAdded } = useAppSelector((state) => state.posts);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    e.stopPropagation();
    if (tags.length) {
      dispatch(addPost({ ...state, tags: tags }));
    } else {
      dispatch(
        openModal({
  ...
  useEffect(() => {
    if (postAdded) {
      dispatcher(initialForm)
      dispatch(
        openModal({ modalText: "post is added", modalType: "infoModal" })
      );
    }
  ...
  if (!id) {
    return <div>owner id is required</div>;
  }
      ...
      <div className="grid  gap-8 grid-cols-1">
        <div className="flex flex-col ">
          <div className="flex flex-col sm:flex-row items-center">
            <h2 className="font-semibold text-lg mr-auto">Create Post</h2>
              ...
              <div className="mb-3 space-y-2 w-full text-xs">
                ...
                <div className="flex gap-2 flex-wrap">
                  {tags.map((item: string) => (
                    <button onClick={() => handleRemoveTag(item)} key={item}>
                      #{item}
                    </button>
                  ))}
                </div>
